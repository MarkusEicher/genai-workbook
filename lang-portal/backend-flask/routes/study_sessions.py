    ...
    try:
      ...
      if 'group_id' not in data or 'study_activity_id' not in data:
        ...
        if 'study_activity_id' not in data:
          missing_fields.append('study_activity_id')
        return jsonify({
      ...
      try:
        ...
        if not cursor.fetchone():
          return jsonify({'error': 'Group not found', 'group_id': data['group_id']}), 400
        
        # Validate study activity exists
        cursor.execute('SELECT id FROM study_activities WHERE id = ?', (data['study_activity_id'],))
        if not cursor.fetchone():
          return jsonify({'error': 'Study activity not found', 'study_activity_id': data['study_activity_id']}), 400
        ...
        # Retrieve the newly created session with all its details
        cursor.execute('''
          SELECT 
            ss.id,
            ss.group_id,
        ...
        if not session:
          app.db.rollback()
          return jsonify({'error': 'Failed to create study session - could not retrieve created session'}), 500
        ...
        app.db.rollback()  # Rollback any database changes if an error occurs
